...
function addEntry(id, amount, description, type, isPinned = false) {
    ...
    pinButton.innerHTML = isPinned ? '<i class="fas fa-thumbtack" style="transform: rotate(45deg);"></i>' : '<i class="fas fa-thumbtack"></i>';
...
function saveToLocalStorage() {
    ...
    banks.forEach(bank => {
        ...
        const entries = Array.from(document.getElementById(bank + '-entries').children).map(entry => ({
            ...
            description: entry.querySelector('strong') ? entry.querySelector('strong').textContent.substring(2) : '',
            type: entry.classList.contains('income') ? 'income' : 'expense',
            pinned: entry.querySelector('.entry-pin').classList.contains('pinned') // Save pin status
        }));
...
function loadFromLocalStorage() {
    ...
    banks.forEach(bank => {
        ...
        entries.forEach(entry => {
            addEntry(bank, parseFloat(entry.amount), entry.description, entry.type, entry.pinned);
        });
    });
}
